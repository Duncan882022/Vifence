...
if [[ "$PRIORITIZE_MODULE05" == "1" ]]; then
  if [[ "$VPS_HC01_ONLY" == "1" ]]; then
    ...
    VPS_PATROL_VEHICLE_DETECT_INTERVAL="${VPS_PATROL_VEHICLE_DETECT_INTERVAL:-3}"
    echo "   Module 05 — chỉ HC-01 (Helmet 01), ${VPS_PATROL_PERSON_MODEL} @ ${VPS_AI_FPS} FPS, ${VPS_AI_MAX_WIDTH}px, vehicle/${VPS_PATROL_VEHICLE_DETECT_INTERVAL}f"
  else
...
PATROL_PERSON_MODEL=${VPS_PATROL_PERSON_MODEL:-yolov8n.pt}
PATROL_VEHICLE_DETECT_INTERVAL=${VPS_PATROL_VEHICLE_DETECT_INTERVAL:-3}
