...
export function AiOperationAlertsPanel({ alerts }: Props) {
  ...
  return (
    <>
      <Panel title="Cảnh báo & đề xuất AI" className="h-full min-h-0" noPadding
        headerRight={(
          <div className="flex items-center gap-1.5">
            <span className="text-[8px] text-muted-foreground/60">{alerts.filter(a => !a.read).length} chưa đọc</span>
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-full bg-primary/15 text-primary text-[8px] font-bold ring-1 ring-primary/25">
              {alerts.length} alert
            </span>
          </div>
        )}
            ...
            return (
              ...
              >
                <div className="shrink-0 pt-0.5">
                  <span className={cn('inline-block w-1.5 h-1.5 rounded-full mt-1', cfg.dot)} />
                </div>
                <div className="flex-1 min-w-0 space-y-0.5">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className="text-[9px] font-semibold text-sky-400">{alert.subject}</span>
                    {!alert.read && (
                      <span className="w-1.5 h-1.5 rounded-full bg-primary shrink-0" />
                    )}
                  </div>
                  <p className="text-[10px] font-semibold text-foreground/90 leading-snug truncate">{alert.title}</p>
                  <p className="text-[9px] text-muted-foreground/60 leading-tight line-clamp-1">{alert.summary}</p>
