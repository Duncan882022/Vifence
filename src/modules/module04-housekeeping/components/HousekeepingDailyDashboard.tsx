...
interface DailyMetricCardProps {
  data: KPIData
  meta: typeof HOUSEKEEPING_METRIC_META[number]
  embedded?: boolean
}
...
interface HousekeepingDailyDashboardProps {
  summary: HousekeepingDailySummary
  embedded?: boolean
}

export function HousekeepingDailyDashboard({ summary, embedded }: HousekeepingDailyDashboardProps) {
    ...
    <div className="grid grid-cols-1 min-[400px]:grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-3">
      {metrics.map((metric, i) => (
        <DailyMetricCard
          key={metric.label}
          data={metric}
          meta={HOUSEKEEPING_METRIC_META[i as MetricIndex]}
          embedded={embedded}
        />
      ))}
    </div>
