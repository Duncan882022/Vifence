import type { TrainingCourseMock } from '@/modules/dao-tao-tuan-thu/data/trainingMockData'
import { courseGroupHasMetrics } from '@/modules/dao-tao-tuan-thu/data/trainingMockData'
import { resolveCourseLocation } from '@/modules/dao-tao-tuan-thu/data/trainingCameras'
import {
...
/**
 * Gộp danh sách base courses (từ API hoặc mock) với custom courses thêm thủ công.
 * @param baseCourses - courses từ store (API hoặc mock fallback)
 * @param customCourses - courses được tạo thủ công trong app
 */
export function getAllTrainingCourses(
  baseCourses: TrainingCourseMock[],
...
export function buildTrainingReport(
  ...
  const rows: TrainingReportRow[] = filtered.map(c => {
    ...
    const compliant = Math.max(0, present - c.exceptions)
    // Tỷ lệ tuân thủ = (có mặt - ngoại lệ) / có mặt
    const rate = present > 0 && courseGroupHasMetrics(c.group)
      ? Math.round((compliant / present) * 100)
      : 0
    return {
      id: c.id,
  ...
  // Tỷ lệ tuân thủ tổng = (ghi nhận - ngoại lệ) / ghi nhận
  const compliantCount = Math.max(0, recorded - exceptions)
  const complianceRate = recorded > 0
    ? Math.round((compliantCount / recorded) * 100)
    : 0
