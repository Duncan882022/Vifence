...
import { buildTrainingPlaybackEventFromRecord } from '../services/trainingPlayback.service'
 ...
 */
export type SessionStatus = 'upcoming' | 'in-session' | 'finished'
...
/** Derive session status from scheduled window vs current clock. */
export function getSessionStatus(courseStart: string, courseEnd: string, courseDate?: string): SessionStatus {
  const now = new Date()
  ...
  if (courseDate) {
    ...
    if (courseDate.includes('-')) {
      // "YYYY-MM-DD"
      const parts = courseDate.split('-').map(Number)
      if (parts.length === 3) {
        y = parts[0]
        m = parts[1] - 1
        d = parts[2]
      }
    } else if (courseDate.includes('/')) {
    ...
    const [startH, startM] = courseStart.split(':').map(Number)
    const [endH, endM] = courseEnd.split(':').map(Number)
    
    const startDateObj = new Date(y, m, d, startH, startM, 0, 0)
    const endDateObj = new Date(y, m, d, endH, endM, 0, 0)
    
    const nowTime = now.getTime()
    if (nowTime >= endDateObj.getTime()) {
      return 'finished'
    } else if (nowTime >= startDateObj.getTime()) {
      return 'in-session'
    } else {
      return 'upcoming'
    }
  ...
  // Fallback to today's date comparisons
  ...
  if (nowMs >= startMs) return 'in-session'
  return 'upcoming'
}
...
const sessionStatusConfig: Record<SessionStatus, { label: string; color: string; bg: string; pulse?: boolean }> = {
  'upcoming':   { label: 'Sắp diễn ra',   color: 'text-amber-400', bg: 'bg-amber-500/15',  pulse: false },
...
export function getAttendanceDetailLine(s: TrainingAttendee): string {
  ...
  const finished = getSessionStatus(s.courseStart, s.courseEnd, s.courseDate) === 'finished'
...
export function formatRecordSessions(r: CourseRecord): string {
  ...
  const finished = getSessionStatus(r.startTime, r.endTime, r.sessionDate) === 'finished'
...
export function SessionBadge({ courseStart, courseEnd, courseDate, small }: { courseStart: string; courseEnd: string; courseDate?: string; small?: boolean }) {
  const ss  = getSessionStatus(courseStart, courseEnd, courseDate)
...
function StudentList({ students, allStudents, tab, onTabChange, onSelect, onSelectContractor, onPlayback, initialLoading = false }: StudentListProps) {
  ...
  const counts = TABS.reduce<Record<string, number>>((acc, t) => {
    acc[t.key] = allStudents.filter(s => matchesTab(s, t.key)).length
    return acc
  }, {})
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <TrainingFilterTabs
        ...
        activeKey={tab}
        onChange={key => onTabChange(key as EventTabKey)}
      />

      {/* Table: header + rows — CourseWorkerList đảm bảo header & rows share 1 grid template */}
      <div className="flex flex-1 flex-col min-h-0 overflow-hidden">
        {initialLoading ? (
            ...
            <div className="flex flex-col gap-px p-1">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="flex items-center gap-2 px-3 py-2.5 animate-pulse">
                  <div className="w-6 h-6 rounded-full bg-[#1e2433]" />
          ...
          <CourseWorkerList
            ...
            items={visible as any[]}
            onSelectItem={(s) => onSelect(s as any)}
            columns={{
              onSelectContractor,
              onPlayback: (courseId, workerId, workerName, courseName) =>
                onPlayback({
                  id: `${workerId}-${courseId}`,
                  time: "",
                  workerName,
                  workerCode: "",
                  workerId,
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="flex items-start gap-3 px-3 py-3 border-b border-[#1e2433]">
          <Avatar name={student.name} color={student.avatarColor} src={student.avatarUrl || getAttendeeAvatarUrl(student.id)} size="lg" />
...
interface ContractorDetailProps {
  company: string
  workers: TrainingAttendee[]
  onBack: () => void
  onSelectStudent: (s: TrainingAttendee) => void
}

function ContractorDetail({ company, workers, onBack, onSelectStudent }: ContractorDetailProps) {
...
// ─── MAIN EXPORT ─────────────────────────────────────────────
import { fetchCourseWorkersAttendance, type CourseWorkerApiItem } from '@/api/course.api'
import { IS_GHPAGES, getGhpagesDemoAttendees } from '../services/ghpagesDemo.service'
...
export function TrainingEventTable({ onPlayback }: TrainingEventTableProps) {
  ...
  const [apiAttendees, setApiAttendees] = useState<TrainingAttendee[]>([])
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      if (IS_GHPAGES) {
        if (!cancelled) {
          setApiAttendees(getGhpagesDemoAttendees('24/06'))
      ...
      try {
        ...
        const response = await fetchCourseWorkersAttendance({
          limit: 100,
        ...
        if (response && response.items) {
          const mapped: TrainingAttendee[] = response.items.map((item: CourseWorkerApiItem) => {
            const workerName = item.worker?.name || 'Chưa rõ'
            const companyName = item.worker?.contractor?.name || 'Nhà thầu phụ'
            const courseName = item.course?.name || 'Khóa đào tạo'
            ...
            const startTimeStr = formatTime(item.course?.startTime || null)
            const endTimeStr = formatTime(item.course?.endTime || null)
            const actualTimeStr = formatTime(item.firstSeenAt)

             // Convert attendanceStatus string từ API sang lower-case status của UI
            let statusVal: AttendanceStatus = 'absent'
            const typeLower = (item.attendanceStatus || '').toLowerCase()
            if (typeLower === 'late') statusVal = 'late'
            else if (typeLower === 'early_leave') statusVal = 'left-early'
            else if (typeLower === 'skip') statusVal = 'skipped'
            else if (typeLower === 'absent') statusVal = 'absent'
            else if (typeLower === 'completed') statusVal = 'completed'
            else if (typeLower === 'insufficient') statusVal = 'insufficient'
            else if (typeLower === 'attending') statusVal = 'attending'
            else if (typeLower === 'away') statusVal = 'away'

            // Tạo history record tương thích
            const historyRecord: CourseRecord = {
              id: item.courseId,
              courseCode: item.course?.code || 'COURSE_CODE',
              courseName: courseName,
              sessionDate: item.course?.startDate ? formatIsoDate(item.course.startDate) : '2026-06-24',
              startTime: startTimeStr,
              endTime: endTimeStr,
              zone: (item.course?.zone || 'OCP1-A') as TrainingZone,
              status: statusVal,
              sessions: item.firstSeenAt ? [{ in: actualTimeStr, out: item.lastSeenAt ? formatTime(item.lastSeenAt) : null }] : [],
              minutesDelta: item.lateMinutes || item.earlyLeaveMinutes || 0
            }

            return {
              id: item.id,
              workerId: item.workerId,
              courseId: item.courseId, // Lưu courseId để xem lại
              employeeCode: item.worker?.phone || 'NV000',
              name: workerName,
              avatarColor: '#3B82F6', // Màu mặc định đẹp
              avatarUrl: item.worker?.faceFrontUrl || undefined,
              company: companyName,
              companyCode: item.worker?.contractor?.code || 'CON-00',
              role: item.worker?.email || 'Học viên',
              currentStatus: statusVal,
              currentCourse: courseName,
              courseDate: item.course?.startDate ? formatDate(item.course.startDate) : '24/06',
              courseStart: startTimeStr,
              courseEnd: endTimeStr,
              actualTime: actualTimeStr,
              exceptionMinutes: item.lateMinutes || item.earlyLeaveMinutes || 0,
              courseHistory: [historyRecord],
              // Trả về trường quét block tương tự CourseWorkerItem
        ...
        console.error("Failed to fetch course workers from API:", err)
  ...
  // Sử dụng dữ liệu thật từ API, không dùng fallback mock data
  const displayAttendees = apiAttendees

  const students = displayAttendees
    .filter(s => matchesTab(s, tab))
    .sort((a, b) => {
      const aLive = getSessionStatus(a.courseStart, a.courseEnd, a.courseDate) === 'in-session' ? 0 : 1
      const bLive = getSessionStatus(b.courseStart, b.courseEnd, b.courseDate) === 'in-session' ? 0 : 1
      return aLive - bLive
    })
  ...
  if (view.kind === 'contractor') {
    const contractorWorkers = displayAttendees.filter(s => s.company === view.company)
    return (
      <ContractorDetail
