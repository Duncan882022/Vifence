...
function ExceptionBreakdown({ stats }: { stats: TrainingDayStats }) {
  return (
    <div className="flex flex-wrap gap-1">
      {EXCEPTION_DISPLAY_ORDER.map((status: AttendanceStatus) => {
        const count = stats.exceptionByStatus[status] ?? 0
...
function DailyMetricCard({
  ...
  const displayPrevious = previousValue ?? 0
  const isUp = changeType === 'increase'
  const isDown = changeType === 'decrease'
  const isNeutral = changeType === 'neutral' || (changeType === undefined && displayChange === 0)
  ...
  return (
    <div className={cn(
      ...
      <div
        ...
        title={`Hôm qua: ${displayPrevious}${unit ? ` ${unit}` : ''}`}
      >
        ...
        <span className={cn(
          ...
          {formatDelta(displayChange, changeUnit)}
        </span>
      </div>
