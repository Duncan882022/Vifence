  ...
  return (
    <div className={cn(
      ...
      <p className="text-[10px] tabular-nums text-foreground">
        {courseGroupHasMetrics(row.group) ? row.present : '—'}
      </p>
      <p className={cn(
...
export function ExecutiveReportPage() {
  ...
  // Local inputs state
  const [tempFromDate, setTempFromDate] = useState(DEFAULT_FROM)
  const [tempToDate, setTempToDate] = useState(DEFAULT_TO)

  // Use the course store directly to fetch with custom params
  const { courses: baseCourses, refetch } = useCourseStore()

  // Load first time
  useEffect(() => {
    const startDateFrom = fromDate ? `${fromDate}T00:00:00Z` : undefined
    const startDateTo = toDate ? `${toDate}T23:59:59Z` : undefined
    void refetch({ startDateFrom, startDateTo })
  }, [refetch])

  const handleFilter = () => {
    setFromDate(tempFromDate)
    setToDate(tempToDate)
    const startDateFrom = tempFromDate ? `${tempFromDate}T00:00:00Z` : undefined
    const startDateTo = tempToDate ? `${tempToDate}T23:59:59Z` : undefined
    void refetch({ startDateFrom, startDateTo })
  }

  const allCourses = useMemo(
    () => getAllTrainingCourses(baseCourses, customCourses),
  ...
  const kpis: KPIData[] = [
    ...
    },
    {
      ...
      unit: 'người',
      detail: report.recorded > 0
        ? `${Math.round((report.exceptions / report.recorded) * 100)}% trên ca đã chạy`
        : undefined,
    ...
    },
    {
      ...
      unit: '%',
      detail: report.recorded > 0
        ? `${report.recorded - report.exceptions}/${report.recorded} học viên`
        : 'Chưa có ca đã chạy',
    },
  ]
