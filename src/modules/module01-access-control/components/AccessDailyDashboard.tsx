...
interface DailyMetricCardProps {
  data: KPIData
  meta: typeof ACCESS_METRIC_META[number]
  embedded?: boolean
}
    ...
    <div className={cn(
      ...
      {showCompare && (
        <div
          ...
          title={`Hôm qua: ${previousValue}`}
        >
          <span className="text-[10px] text-muted-foreground whitespace-nowrap truncate min-w-0 tabular-nums">
            <span className="font-semibold text-muted-foreground/90">{previousValue}</span>
            <span className="text-muted-foreground/50 ml-1">hôm qua</span>
          </span>
          <span className={cn(
...
interface AccessDailyDashboardProps {
  summary: AccessDailySummary
  embedded?: boolean
}

export function AccessDailyDashboard({ summary, embedded }: AccessDailyDashboardProps) {
    ...
    <div className="grid grid-cols-1 min-[400px]:grid-cols-2 xl:grid-cols-4 gap-2.5 sm:gap-3">
      {metrics.map((metric, i) => (
        <DailyMetricCard
          key={metric.label}
          data={metric}
          meta={ACCESS_METRIC_META[i as MetricIndex]}
          embedded={embedded}
        />
      ))}
    </div>
