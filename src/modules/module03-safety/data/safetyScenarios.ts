...
import { isImplementedSafetyScenario } from './implementedSafetyCatalog'
...
const SCENARIO_ID_DISPLAY_NAME: Record<string, string> = {
  ...
  'BPTC-001': 'Lưới bao che thiếu/bẩn',
...
if (import.meta.env.DEV) {
  for (const def of RAW_SCENARIO_DEFS) {
    if (!isImplementedSafetyScenario(def.id)) {
      console.error(`[safetyScenarios] ${def.id} chưa có trong implementedSafetyCatalog`)
    }
  }
