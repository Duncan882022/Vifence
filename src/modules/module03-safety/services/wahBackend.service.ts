...
export type WahBehavior = 'person' | 'no_harness' | 'safety_harness'

export interface WahDetection {
  behavior: WahBehavior
  label: string
  confidence: number
  bbox: [number, number, number, number]
}

export interface WahResult {
  camera_id: string
  width: number
  height: number
  detections: WahDetection[]
...
export function buildWahAnalyzeUrl(baseUrl: string): string {
  return `${normalizeBaseUrl(baseUrl)}/analyze/wah/frame`
}
...
export async function postWahAnalyzeFrame(
  backendUrl: string,
  cameraId: string,
  image: string,
): Promise<WahResult> {
  const res = await fetchWithTimeout(buildWahAnalyzeUrl(backendUrl), {
    method: 'POST',
    headers: {
      ...TUNNEL_HEADERS,
...
export interface WahClientOptions {
  cameraId: string
  backendUrl?: string
  onResult: (result: WahResult) => void
  onStatusChange: (status: MobileAiConnectionStatus, message?: string) => void
  shouldAnalyze?: () => boolean
  intervalMs?: number
}

export function createWahClient(
  video: HTMLVideoElement,
  options: WahClientOptions,
): { stop: () => void } {
  const {
    cameraId,
  ...
  const tick = async () => {
    ...
    inFlight = true
    try {
      const result = await postWahAnalyzeFrame(backendUrl, cameraId, image)
      if (stopped) return
      connectedOnce = true
