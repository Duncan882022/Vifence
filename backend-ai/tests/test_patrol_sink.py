...
class PatrolSinkTests(unittest.TestCase):
    ...
    def test_no_face_creates_object(self) -> None:
        oid = sink.record_observation(camera_id="HC-01", track_id="ptk0001:person", now=1_000.0)
    ...
    def test_same_track_reuses_object(self) -> None:
        a = sink.record_observation(camera_id="HC-01", track_id="ptk0001:person", now=1_000.0)
        b = sink.record_observation(camera_id="HC-01", track_id="ptk0001:person", now=1_010.0)
    ...
    def test_face_promotes_object_and_keeps_history(self) -> None:
        ...
        sink.record_observation(camera_id="HC-01", track_id="ptk0002:person", now=1_000.0)
    ...
    def test_turning_away_does_not_fall_back_to_object(self) -> None:
        ...
        sink.record_observation(
            camera_id="HC-01", track_id="ptk0003:person",
            face_embedding=_vec(2), face_quality=0.9, now=1_000.0,
        )
        again = sink.record_observation(
            camera_id="HC-01", track_id="ptk0003:person", now=1_050.0,
        )
        self.assertTrue(str(again).startswith("pers-"))
        self.assertEqual(daystore.list_objects(db.today_vn(1_000.0)), [])
    ...
    def test_same_person_two_cameras_one_card(self) -> None:
        ...
        a = sink.record_observation(
            ...
            face_embedding=_vec(5), face_quality=0.9, now=1_000.0,
        )
        b = sink.record_observation(
            camera_id="HC-02", track_id="ptk0009:person",
            face_embedding=_vec(5), face_quality=0.9, now=2_000.0,
        )
        self.assertEqual(a, b)
    ...
    def test_reencounter_updates_time_not_new_card(self) -> None:
        pers = sink.record_observation(
            camera_id="HC-01", track_id="ptk0001:person",
            face_embedding=_vec(7), face_quality=0.9, now=1_000.0,
        )
        sink.record_observation(
            camera_id="HC-01", track_id="ptk0044:person",
            face_embedding=_vec(7), face_quality=0.9, now=9_000.0,
        )
    ...
    def test_identified_name_survives_new_track(self) -> None:
        ...
        pers = sink.record_observation(
            camera_id="HC-01", track_id="ptk0001:person",
            face_embedding=_vec(9), face_quality=0.9, now=1_000.0,
        )
        ...
        again = sink.record_observation(
            ...
            face_embedding=_vec(9), face_quality=0.9, now=90_000.0,
        )
